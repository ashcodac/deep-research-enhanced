...
function OutputTemplateUpload({ onFilesChange }: { onFilesChange: (files: File[]) => void }) {
  ...
  const handleFilesChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const updatedFiles = [...templateFiles, ...selected];
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    const updatedFiles = [...templateFiles, ...dropped];
    setTemplateFiles(updatedFiles);
    onFilesChange(updatedFiles);
  };
  ...
  const removeTemplateFile = (index: number) => {
    const updated = templateFiles.filter((_, i) => i !== index);
    setTemplateFiles(updated);
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        <p className="text-base text-gray-600 dark:text-gray-300 mb-2">
          Select or Drag Output Templates (PDF, TXT, MD)
        </p>
...
export default OutputTemplateUpload;
